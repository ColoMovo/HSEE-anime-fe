...
var getRemotionEnvironment = () => {
  ...
  const isRendering = typeof window !== "undefined" && typeof window.process !== "undefined" && typeof window.process.env !== "undefined" && (window.process.env.NODE_ENV === "test" || window.process.env.NODE_ENV === "production" && typeof window !== "undefined" && typeof window.remotion_puppeteerTimeout !== "undefined");
...
// src/audio/Audio.tsx
import {forwardRef as forwardRef5, useCallback as useCallback7, useContext as useContext22} from "react";
...
// src/loop/index.tsx
import React8, {createContext as createContext10, useMemo as useMemo11} from "react";
import {
jsx as jsx10
} from "react/jsx-runtime";
var LoopContext = createContext10(null);
var useLoop = () => {
...
var Loop = ({ durationInFrames, times = Infinity, children, name, ...props }) => {
  ...
  return jsx10(LoopContext.Provider, {
    value: loopContext,
    children: jsx10(Sequence, {
      durationInFrames,
      from,
      name: name ?? "<Loop>",
      _remotionInternalLoopDisplay: loopDisplay,
...
import {createContext as createContext11, useEffect as useEffect6, useState as useState6} from "react";
import {
jsx as jsx11
} from "react/jsx-runtime";
var PreloadContext = createContext11({});
var preloads = {};
var updaters = [];
...
var PrefetchProvider = ({ children }) => {
  const [_preloads, _setPreloads] = useState6(() => preloads);
  useEffect6(() => {
...
import {createContext as createContext12, useMemo as useMemo12, useReducer} from "react";
import {
jsx as jsx12
} from "react/jsx-runtime";
...
var DurationsContext = createContext12({
  durations: {},
  setDurations: () => {
...
import {useContext as useContext14, useMemo as useMemo14} from "react";

// src/buffering.tsx
import React11, {useCallback as useCallback4, useEffect as useEffect7, useMemo as useMemo13, useRef as useRef2, useState as useState7} from "react";
import {
jsx as jsx13
} from "react/jsx-runtime";
var useBufferManager = () => {
  const [blocks, setBlocks] = useState7([]);
  const [onBufferingCallbacks, setOnBufferingCallbacks] = useState7([]);
  const [onResumeCallbacks, setOnResumeCallbacks] = useState7([]);
  const buffering = useRef2(false);
  const addBlock = useCallback4((block) => {
    setBlocks((b) => [...b, block]);
    return {
      unblock: () => {
  ...
  const listenForBuffering = useCallback4((callback) => {
  ...
  const listenForResume = useCallback4((callback) => {
...
// src/use-buffer-state.ts
var useBufferState = () => {
  const buffer = useContext14(BufferingContextReact);
  return useMemo14(() => ({
    delayPlayback: () => {
      if (!buffer) {
...
import {useContext as useContext16, useEffect as useEffect9, useMemo as useMemo15, useState as useState8} from "react";
...
import {useContext as useContext17, useEffect as useEffect10} from "react";
...
var useAppendVideoFragment = ({
  ...
  const actualFromRef = useRef3(initialActualFrom);
  const actualDuration = useRef3(initialDuration);
  const actualSrc = useRef3(initialActualSrc);
...
var useMediaPlayback = ({
  ...
  const absoluteFrame = useTimelinePosition();
  const [playing] = usePlayingState();
  const buffering3 = useContext17(BufferingContextReact);
  ...
  const pausedOrBuffering = !playing || buffering3 && buffering3.buffering.current;
  useEffect10(() => {
...
var useMediaTagVolume = (mediaRef) => {
  const [actualVolume, setActualVolume] = useState9(1);
  useEffect11(() => {
...
import {createContext as createContext13, useContext as useContext18, useMemo as useMemo16} from "react";
var MediaVolumeContext = createContext13({
  mediaMuted: false,
  mediaVolume: 1
});
var SetMediaVolumeContext = createContext13({
  setMediaMuted: () => {
    throw new Error("default");
  },
  setMediaVolume: () => {
...
var SharedAudioContext = createContext14(null);
var SharedAudioContextProvider = ({ children, numberOfAudioTags, component }) => {
  ...
  const registerAudio = useCallback5((aud, audioId) => {
  ...
  const unregisterAudio = useCallback5((id) => {
    const cloned = [...takenAudios.current];
  ...
  const updateAudio = useCallback5(({
    aud,
    audioId,
    id
  }) => {
  ...
  const playAllAudios = useCallback5(() => {
    refs.forEach((ref) => {
...
var AudioForDevelopmentForwardRefFunction = (props, ref) => {
  const [initialShouldPreMountAudioElements] = useState11(props.shouldPreMountAudioTags);
  ...
  const currentOnDurationCallback = useRef5();
...
var RenderAssetManagerProvider = ({ children }) => {
  const [renderAssets, setRenderAssets] = useState12([]);
...
var AudioRefForwardingFunction = (props, ref) => {
  ...
  const onDuration = useCallback7((src, durationInSeconds) => {
...
// src/Composition.tsx
import {Suspense, useContext as useContext24, useEffect as useEffect17, useMemo as useMemo23} from "react";
import {createPortal} from "react-dom";

// src/Folder.tsx
import {createContext as createContext16, useContext as useContext23, useEffect as useEffect16, useMemo as useMemo21} from "react";
...
var Folder = ({
  ...
  const parent = useContext23(FolderContext);
  const { registerFolder, unregisterFolder } = useContext23(CompositionManager);
...
import React16, {useMemo as useMemo22} from "react";
...
var ClipComposition = ({ children }) => {
  const { clipRegion } = useContext24(NativeLayersContext);
...
var CompositionManagerProvider = ({ children, numberOfAudioTags }) => {
  ...
  const currentcompositionsRef = useRef7(compositions);
  ...
  const updateCompositions = useCallback8((updateComps) => {
  ...
  const registerComposition = useCallback8((comp) => {
  ...
  const unregisterComposition = useCallback8((id) => {
    setCompositions((comps) => {
      return comps.filter((c) => c.id !== id);
    });
  }, []);
  const registerFolder = useCallback8((name, parent) => {
  ...
  const unregisterFolder = useCallback8((name, parent) => {
    setFolders((prevFolders) => {
...
// src/IFrame.tsx
import {forwardRef as forwardRef6, useCallback as useCallback9, useState as useState14} from "react";
import {
jsx as jsx23
} from "react/jsx-runtime";
var IFrameRefForwarding = ({
  ...
  const didLoad = useCallback9((e) => {
    continueRender(handle);
    onLoad?.(e);
  }, [handle, onLoad]);
  const didGetError = useCallback9((e) => {
    continueRender(handle);
    if (onError) {
      onError(e);
    } else {
...
var ImgRefForwarding = ({
  ...
  const retryIn = useCallback10((timeout) => {
    if (!imageRef.current) {
      return;
    }
...
import React23, {useMemo as useMemo26} from "react";
...
import {Children, forwardRef as forwardRef8, useMemo as useMemo27} from "react";
...
import {useCallback as useCallback11, useContext as useContext26, useEffect as useEffect19, useMemo as useMemo28} from "react";
import {
jsx as jsx29
} from "react/jsx-runtime";
...
var VideoForDevelopmentRefForwardingFunction = (props2, ref) => {
  ...
  const currentOnDurationCallback = useRef10();
  currentOnDurationCallback.current = onDuration;
  useEffect20(() => {
    const { current } = videoRef;
    if (!current) {
...
var OffthreadVideo = (props2) => {
  ...
  const environment = getRemotionEnvironment();
  const onDuration = useCallback12(() => {
    return;
  }, []);
...
var VideoForwardingFunction = (props2, ref) => {
  ...
  const { loop: loop4, ...propsOtherThanLoop } = props2;
  const { fps } = useVideoConfig();
  const environment = getRemotionEnvironment();
